...
const steps = {
  indexing: [
    ...
    { icon: 'EK', label: 'Embedding', desc: '将文本转化为 1536 维的向量 (Vector)' },
    { icon: 'SJ', label: '向量存储', desc: '存入 Vector DB，构建索引以便快速搜索' },
  ],
  retrieval: [
    { icon: 'YR', label: '用户提问', desc: '用户输入自然语言问题 (Query)' },
    { icon: 'EK', label: 'Embedding', desc: '将问题也转化为同维度的向量' },
    { icon: 'JS', label: '相似度搜索', desc: '在库中计算 Cosine Similarity，找出最相似的 N 个片段' },
