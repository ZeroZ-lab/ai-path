...
<!--
欢迎来到**逻辑部门**，这里负责指挥交通和批量处理。

1.  **If-Else (红绿灯)**：这是最常见的。如果文件上盖了红章，走左边；盖了蓝章，走右边。做一次判断，定一个方向。

2.  **Iteration (批量盖章机)**：当你有一堆发票要处理时，不要一张一张建流程。用 Iteration，它能像点钞机一样，一次性把 100 张发票（数组）都盖上章。
    *   **关键点**：处理的是**一堆同类的东西**。

3.  **Loop (返工重修)**：这是最严厉的质检员。如果写出来的文章不合格，它会把你打回去重写，直到写合格为止，才能放你走。
    *   **关键点**：是为了**达成某个目标**而反复做。
-->
