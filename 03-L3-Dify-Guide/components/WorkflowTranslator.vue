...
const steps = [
  { id: 1, text: '收到需求', node: 'Start', icon: '🏁', type: 'trigger' },
  { id: 2, text: '判断金额', node: 'If-Else', icon: '🔀', type: 'logic' },
  { id: 3, text: '查询条款', node: 'Knowledge', icon: '📚', type: 'rag' },
  { id: 4, text: '生成合同', node: 'LLM', icon: '🤖', type: 'gen' },
]
...
<template>
  <div class="w-full h-full flex flex-col gap-6 select-none font-sans p-2">
    ...
    <div class="flex justify-between items-center px-4">
      ...
      >
        <span>{{ currentStep === steps.length ? '↺ 重置' : '👇 下一步' }}</span>
      </button>
    </div>
