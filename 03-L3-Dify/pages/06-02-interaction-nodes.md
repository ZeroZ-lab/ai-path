...
<!--
欢迎来到**交互部门**，这里是律所的接待大厅。

1.  **Start (开始)**：就是律所的**大门**。只有当事人带着需求来了（触发），整个法律服务流程才会启动。
2.  **User Input (输入)**：就像**接案笔录**。律师需要当事人详细填写案件信息，比如“对方姓名”、“涉案金额”、“诉求”。
3.  **Answer (回复)**：这是**口头咨询**。律师坐在当事人面前，一边分析一边说出建议（流式输出），互动性很强。
4.  **End (结束)**：这是**出具书面意见**。律师将最终分析结果整理成一份正式的《法律意见书》（JSON），盖章交付给客户或发给法院系统。

记住：Start 和 End 是必须有的，中间的 Input 和 Answer 看业务场景选，是要做咨询（Answer）还是要出文书（End）。
-->
